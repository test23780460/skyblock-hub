...
- The production vinext build passes; 126 automated tests pass with zero failures: 40 engine, 16 service, 50 provider/security, and 20 rendered HTML.
...
- The secret-pattern scan passes across 259 project files.
...
- [ ] `[PROD]` Apply configurable TTL classes for static metadata, request-driven profiles, shared Bazaar data, and centralized auctions. Eliminate direct page-to-Hypixel calls. **Implemented subset:** public economy pages now read D1 snapshots only; player cache/rate state remains per-runtime and TTLs are not centrally configurable. (Req. 20, 83)
...
- [ ] `[PROD]` Build meaningful unit/integration/API/progression/calculator/economy/valuation/parser/auth/permission suites on fixtures. **Implemented subset:** 126 passing tests cover engines, services, providers/security, D1-backed saved state/history, NBT bounds, AI grounding/metrics, workers, and rendered routes; full browser E2E, live providers, and several deep feature/auth flows remain. (Req. 119)
...
- [x] `[PROD]` Prepare a stable, portable Sites deployment without damaging external-host support. Isolate unsupported workers rather than removing features. **Evidence:** an earlier owner-only Sites build exists; safe feature gates and external-host/portability paths remain documented. The current workspace delta is not deployed. (Req. 89, 139, 140, 141)
