...
- [ ] `[EXTERNAL]` Activate live Hypixel requests only after a replacement key is installed on the private gateway Worker, the shared signing secret and exact gateway URL are installed in Sites, `REQUIRE_PLAYER_GATEWAY=true`, the Hypixel application has the required approval, and a deployed username/UUID smoke test succeeds. Otherwise retain the labeled demo and explicit unavailable state. (Req. 3, 77, 80, 99, 147)
