...
- The production vinext build passes; 153 automated tests pass with zero failures: 40 engine, 16 service, 72 provider/security, and 25 rendered HTML.
- The Drizzle history check and `npm run db:smoke` pass; four migrations create 36 SQLite tables with zero foreign-key-check findings in an isolated in-memory database.
- The secret-pattern scan passes across 270 project files.
- CI, Docker, hosting metadata, schema/repositories, policy documentation, and release documentation exist in the workspace; no current-source commit/GitHub run, local Docker runtime smoke, production database, public deployment, or external activation is claimed by this evidence.
...
- [ ] `[PROD]` Implement worker jobs as scheduler-independent tasks for Bazaar refresh, auction/ended-sale ingestion, hourly/daily aggregates, valuations, cache cleanup, and maintenance. Make the worker externally deployable. **Implemented subset:** a scheduled, lease-fenced, backoff-aware D1 cycle publishes complete Bazaar, active-Auction, and ended-sale snapshots plus idempotent Bazaar hour/day buckets with bounded retention; valuation, other-resource, broader maintenance, and external-host composition remain. (Req. 95, 96, 140)
...
- [ ] `[PROD]` Implement one server-only Hypixel client with endpoint adapters, queue, shared cache, rate-limit manager, backoff, typed errors, metrics, and correct `429` handling; never use multiple keys to evade limits. **Implemented subset:** authenticated player traffic is isolated behind a signed fixed-route Worker, normalized Workers KV, per-actor/shared abuse guards, bounded transports, typed errors, and Hypixel header backoff. An owner-only Sites compatibility mode issues 30-second exact-body, origin-bound browser capabilities plus ten-minute per-profile save receipts, but capabilities are replayable during their window; authoritative consume-once/global credential budgeting and complete durable metrics remain. (Req. 80, 82, 84)
...
- [ ] `[PROD]` Apply configurable TTL classes for static metadata, request-driven profiles, shared Bazaar data, and centralized auctions. Eliminate direct page-to-Hypixel calls. **Implemented subset:** hosted player/provider values use normalized Workers KV with bounded fresh/stale TTLs and per-isolate L0 caching; public economy pages read D1 snapshots only. TTLs are not yet centrally configurable, and cross-region in-flight/backoff coordination remains approximate. (Req. 20, 83)
...
- [ ] `[PROD]` Ingest permitted Bazaar, auctions, ended auctions, items, Collections, skills, and other public resources through centralized workers. **Implemented subset:** one elected worker persists normalized Bazaar, complete active-Auction, and ended-sale feeds; the remaining resource feeds are absent. (Req. 85)
...
- [ ] `[PROD]` Implement optional accounts around canonical internal users, replaceable auth identities, saved usernames/accounts/profiles, preferences, favorites, builds, goals, recommendation state, and appropriate AI history. **Implemented subset:** canonical Sites identity plus owner-scoped saved accounts/profiles, preferences, favorites, builds/sharing, goals, and deletion work end to end; browser live-profile saves use a narrow ten-minute Worker-signed receipt instead of a second lookup. Recommendation actions, any justified AI history, and portable external auth remain. (Req. 13, 19, 94)
...
- [ ] `[PROD]` Build meaningful unit/integration/API/progression/calculator/economy/valuation/parser/auth/permission suites on fixtures. **Implemented subset:** 153 passing tests cover engines, services, signed gateway/provider/security boundaries, D1-backed saved state/history, NBT bounds, AI grounding/metrics, workers, legal pages, feature-aware navigation, and rendered routes; full browser E2E, live providers, and several deep feature/auth flows remain. (Req. 119)
...
- [x] `[PROD]` Configure CI for deterministic install, lint, typecheck, tests, and production build; keep credentials and generated junk out of Git. **Evidence:** `.github/workflows/ci.yml`, `.gitignore`, blank `.env.example`, local validation, and prior successful GitHub Actions run `31294213800`; no current-source remote run is inferred. (Req. 137, 138)
...
- [x] `[PROD]` Prepare a stable, portable Sites deployment without damaging external-host support. Isolate unsupported workers rather than removing features. **Evidence:** an earlier owner-only Sites build exists; safe feature gates and external-host/portability paths remain documented. The current validated branch source is not yet deployed. (Req. 89, 139, 140, 141)
...
- [ ] `[EXTERNAL]` Activate live Hypixel requests only after a replacement key is installed on the private gateway Worker, the shared signing secret and exact gateway URL are installed in Sites, `REQUIRE_PLAYER_GATEWAY=true`, the Hypixel application has the required Production approval, and deployed username/UUID and failure-path smoke tests succeed. If owner-only browser capability mode is required, also align exact `SITE_URL`/`SKYPILOT_SITE_ORIGIN`, enable `ENABLE_BROWSER_PLAYER_GATEWAY`, verify CORS/hostile-origin/exact-body behavior, and prove a selected profile saves only with its valid ten-minute receipt. Public access additionally requires authoritative capability/credential coordination or explicit residual-risk acceptance plus final release QA. Otherwise retain the labeled demo and explicit unavailable state. (Req. 3, 77, 80, 99, 147)
