...
- The production vinext build passes; 169 automated tests pass with zero failures: 40 engine, 16 service, 88 provider/security, and 25 rendered HTML.
...
- The secret-pattern scan passes across 275 tracked project files.
- Commit `3b4064d` is pushed to the draft PR and GitHub Actions run `31775265691` passes validation and Docker jobs. The same source is deployed owner-only to Sites; live `Justiwantdreams` lookup and receipt-backed profile save passed. Public access, the economy schedule, Hypixel Production approval, custom-domain activation, and broad release QA are not claimed.
...
- [ ] `[PROD]` Build meaningful unit/integration/API/progression/calculator/economy/valuation/parser/auth/permission suites on fixtures. **Implemented subset:** 169 passing tests cover engines, services, signed gateway/provider/security boundaries, D1-backed saved state/history, NBT bounds, AI grounding/metrics, workers, legal pages, feature-aware navigation, and rendered routes; automated browser E2E and several deep feature/auth flows remain. One owner-authenticated live lookup/save flow is separately verified. (Req. 119)
...
- [x] `[PROD]` Configure CI for deterministic install, lint, typecheck, tests, and production build; keep credentials and generated junk out of Git. **Evidence:** `.github/workflows/ci.yml`, `.gitignore`, blank `.env.example`, local validation, and successful exact-head GitHub Actions run `31775265691` for `3b4064d`. (Req. 137, 138)
...
- [x] `[PROD]` Prepare a stable, portable Sites deployment without damaging external-host support. Isolate unsupported workers rather than removing features. **Evidence:** current commit `3b4064d` is deployed owner-only on Sites with the separate gateway Worker, safe feature gates, documented external-host paths, and a successful live lookup/profile-save smoke. Public access and remaining release gates stay separate. (Req. 89, 139, 140, 141)
...
- [ ] `[EXTERNAL]` Before public live Hypixel access, confirm the application has Production approval and complete the public abuse/risk and final release gates. **Owner-only evidence:** the replacement key is Worker-only; the shared signing secret, exact gateway URL/origins, `REQUIRE_PLAYER_GATEWAY`, and browser capability flag are active; allowed and hostile CORS checks passed; a deployed `Justiwantdreams` lookup and ten-minute receipt-backed save succeeded. Public access still requires authoritative capability/credential coordination or explicit residual-risk acceptance plus final release QA. (Req. 3, 77, 80, 99, 147)
