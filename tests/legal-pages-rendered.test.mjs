...
test("privacy notice renders the implemented data boundaries and controls", async () => {
  ...
  assert.match(html, /EFFECTIVE.{0,24}AUGUST 12, 2026/i);
  ...
  assert.match(html, /logs only the request event.*response status.*duration/is);
  assert.match(html, /Public economy data is separate/i);
  assert.match(html, /does not delete the user.{0,20}ChatGPT account/is);
...
test("terms render independent-project, fair-use, and no-guarantee limits", async () => {
  ...
  const html = await response.text();
  assert.match(html, /EFFECTIVE.{0,24}AUGUST 12, 2026/i);
  assert.match(html, /independently operated.*not an official product of.*Hypixel Inc\..*Mojang Studios.*Microsoft.*OpenAI/is);
