...
export class KvTtlCache implements TtlCache {
  ...
  async get<T>(key: string): Promise<CacheLookup<T> | null> {
    ...
    // Do not rehydrate MemoryTtlCache from KV here: its public set contract
    // assigns a new storedAt, which would make an older shared snapshot appear
    // freshly fetched after a cold isolate read. L0 still serves values written
    // by this isolate without rewriting source freshness metadata.
    return {
  ...
  async deleteByPrefix(): Promise<number> {
    // The player gateway never performs broad cache invalidation; deleting
    // global key ranges would turn a bounded user action into a bulk KV scan.
    return 0;
  }
