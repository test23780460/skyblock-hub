...
| `ENABLE_PUBLIC_ECONOMY` | `false` | Enforced by Bazaar and Auction APIs. Keep off until one scheduled ingestion worker and a durable snapshot sink are active. |
...
`DB` is a platform binding, not a secret placed in `.env`. R2/object storage is not currently configured.
