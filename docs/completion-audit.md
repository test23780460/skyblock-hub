...
| 89 | Initial hosting target | **Partial** | Native production/staging web Workers and matching private economy Workers exist with isolated app D1/KV/rate bindings, a shared `PROVIDER_BUDGET_DB`, and `ECONOMY_SERVICE`. Exact application commit `75659fb2f3d6` has a recorded staging web version plus disabled-economy service-binding/API/browser smoke, and production application D1 is fully migrated. The private production economy Worker is deployed from `4cf0a9708efc` as version prefix `01057fa0` with economy off, no public target, and no Cron. Production web deployment/service-binding smoke, Workers Builds, domain, remaining database-role/backup evidence, and broader production QA remain. The Sites deployment is rollback history. |
