...
- `/dashboard?player=<username-or-java-uuid>` performs a request-driven lookup
  ...
  Focused tests, the public privacy disclosure, and a recorded exact-commit
  staging egress/schema/error smoke pass. That smoke used an invalid configured Hypixel
  credential, so it did not produce successful live player data.
...
passing focused provider tests, and the recorded staging lookup where both
official hosts fail. Before public activation, repeat with a valid rotated
Hypixel credential and verify final UUID/display-name agreement.
