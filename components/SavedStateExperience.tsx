    ...
    <section className={`${styles.stack} ${preferences.compactAccountView ? styles.compact : ""}`} aria-label="Saved SkyPilot workspace">
      ...
      <div className={styles.grid}>
        ...
        <section className="panel"><div className="panel-header"><div><h2>Saved profiles</h2><small>Links only · no background refresh</small></div><span>{state?.profiles.length ?? 0}</span></div><div className={styles.panelBody}>
          <div className={styles.list}>{state?.profiles.length ? state.profiles.map((profile) => <article className={styles.card} key={profile.id}><div className={styles.cardHead}><div><h3>{profile.alias || profile.name}</h3><p>{profile.username} · {profile.name} · {profile.gameMode}</p></div>{profile.isPinned ? <span className={styles.badge}>PINNED</span> : null}</div><form className={styles.actions} onSubmit={(event) => void renameProfile(event, profile)}><label className="sr-only" htmlFor={`profile-alias-${profile.id}`}>Label for {profile.name}</label><input className="control" id={`profile-alias-${profile.id}`} name="alias" defaultValue={profile.alias ?? ""} maxLength={60} placeholder="Optional profile label" /><button type="submit" disabled={Boolean(busy)}>Save label</button></form><div className={styles.actions}><Link href={`/dashboard?player=${encodeURIComponent(profile.username)}&profile=${encodeURIComponent(profile.id.replace(/^profile_/u, ""))}`}>Open saved profile</Link><button type="button" disabled={Boolean(busy)} onClick={() => void mutate(`/api/saved-profiles/${profile.id}`, "PATCH", { isPinned: !profile.isPinned }, profile.isPinned ? "Profile unpinned." : "Profile pinned.")}>{profile.isPinned ? "Unpin" : "Pin"}</button><button className={styles.danger} type="button" disabled={Boolean(busy)} onClick={() => window.confirm(`Remove ${profile.alias || profile.name} from saved profiles?`) && void mutate(`/api/saved-profiles/${profile.id}`, "DELETE", undefined, "Saved profile removed.")}>Remove</button></div></article>) : <p className={styles.muted}>Saved profiles appear here after an explicit live lookup. Saving a link does not schedule monitoring.</p>}</div>
        </div></section>
      </div>
