...
export function AuctionExperience() {
  ...
  return <div className="page-shell auction-page">
    ...
    <section className="market-toolbar panel auction-toolbar"><form onSubmit={search}><label><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the active snapshot by item name" aria-label="Search active auction snapshot" /></label><button type="submit">Search snapshot</button></form><div role="group" aria-label="Sort auctions">{(["ending", "price-low", "price-high", "name"] as AuctionSort[]).map((value) => <button aria-pressed={sort === value} className={sort === value ? "active" : ""} type="button" onClick={() => setSort(value)} key={value}>{value.replace("-", " ")}</button>)}</div></section>
